...
def test_closed_children_do_not_complete_in_progress_parent():
    ...
    responses = {
        ("show", "escapement-e3ai"): [
            {"id": "escapement-e3ai", "status": "in_progress"}
        ],
...
def test_open_requested_root_blocks_for_its_own_canonical_status(root_id):
    ...
    calls: list[list[str]] = []
    responses = _responses([{"id": root_id, "status": "in_progress"}], root_id=root_id)

    def run_bd(args):
        calls.append(args)
        return responses.get(tuple(args))

    assert _check_task_scope(_mode(root_id), run_bd) == (
        "block",
...
def test_compatibility_alias_queries_root_before_allowing_drain(tmp_path):
    ...
    assert stop_hook._check_task_mode_queue(
        _mode(ROOT, str(tmp_path)), run_bd=run_bd
    ) == (
        "allow",
        "queue_drained",
    )
...
def _run_stop(
    ...
    *,
    ...
    (thread_dir / "session_mode.json").write_text(
        json.dumps({**_mode(ROOT, str(repo_cwd)), "session_id": session_id}),
        encoding="utf-8",
    )
    ...
    monkeypatch.setattr(
        stop_hook.session_isolation, "write_checkout", lambda *args: None
    )
...
def test_public_stop_allows_closed_root_with_empty_descendants(
